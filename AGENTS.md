...
**Index build rules** (offline, by Codex in a working session — not at app runtime):
...
1. **Route** — send the question + the index (it's small) to Codex; it returns the set of relevant report ids and, where computable, exact counts straight from index data.
...
9. ✅ DONE (2026-06-11) — Local-model provider + usage meter: Settings toggle between Codex cloud and any OpenAI-compatible endpoint (Ollama/vLLM/LiteLLM) for on-prem demos; exact per-provider token accounting priced at sonnet-4-6 rates ("saved vs Codex" for local traffic). Spec + acceptance: BUILD_PLAN.md step 9. PENDING: one live run against a real local endpoint to confirm the OpenAI streaming parser.
